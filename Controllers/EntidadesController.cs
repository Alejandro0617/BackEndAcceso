  ...
  public class EntidadesController: ControllerBase // Define un controlador llamado `EntidadesController`.
  {
    ...
    public EntidadesController(ControlConexion controlConexion, IConfiguration configuration) {
      this.controlConexion = controlConexion ??
        throw new ArgumentNullException(nameof(controlConexion));
    ...
    public IActionResult Listar(string projectName, string tableName) // Método que lista todas las filas de una tabla dada.
    {
      ...
      try {
        var lista = new List < Dictionary < string,
          object ? >> (); // Crea una lista para almacenar las filas resultantes.
    ...
    public IActionResult GetByKey(string projectName, string tableName, string keyName, string value) // Método que obtiene una fila específica basada en una clave.
    {
      ...
      try {
        string provider = _configuration["DatabaseProvider"] ??
          throw new InvalidOperationException("DatabaseProvider not configured."); // Obtiene el proveedor de base de datos desde la configuración.
        ...
        var resultado = controlConexion.EjecutarConsultaSql(comandoSQL, new DbParameter[] {
          parametro
    ...
    private object ? ConvertJsonElement(JsonElement jsonElement) {
      ...
      case JsonValueKind.Number:
        return jsonElement.TryGetInt32(out
          var intValue) ? (object) intValue : jsonElement.GetDouble();
    ...
    public IActionResult Crear(string projectName, string tableName, [FromBody] Dictionary < string, object ? > entidadData) // Crea una nueva fila en la tabla especificada.
    {
      ...
      try {
        ...
        var passwordKeys = new [] {
          "password",
          "contrasena",
          "passw"
        }; // Lista de posibles nombres para campos de contraseña.
        ...
        string provider = _configuration["DatabaseProvider"] ??
          throw new InvalidOperationException("DatabaseProvider not configured."); // Obtiene el proveedor de base de datos.
    ...
    public IActionResult Eliminar(string projectName, string tableName, string keyName, string keyValue) // Elimina una fila de la tabla basada en una clave.
    {
      ...
      try {
        ...
        controlConexion.AbrirBd(); // Abre la conexión a la base de datos.
        controlConexion.EjecutarComandoSql(comandoSQL, new [] {
          parametro
        }); // Ejecuta la consulta SQL para eliminar la fila.
        controlConexion.CerrarBd(); // Cierra la conexión a la base de datos.
	...
	[AllowAnonymous] // Solo permite que un usuario autorizado acceda
    [HttpPost("encriptar-contrasena-usuario")] // Ruta temporal para encriptar la contraseña del usuario específico
    public IActionResult EncriptarContrasenaUsuario()
    {
        try
        {
            controlConexion.AbrirBd();
        
            // Consulta para obtener la contraseña del usuario específico
            string comandoSelect = "SELECT email, contrasena FROM usuario WHERE email = @Email";
            var parametrosSelect = new DbParameter[]
            {
                CreateParameter("@Email", "usuario@gmail.com")
            };
        
            var resultado = controlConexion.EjecutarConsultaSql(comandoSelect, parametrosSelect);

            if (resultado.Rows.Count > 0)
            {
                DataRow fila = resultado.Rows[0];
                string email = fila["email"].ToString();
                string contraseña = fila["contrasena"].ToString();
                string hashedPassword = BCrypt.Net.BCrypt.HashPassword(contraseña); // Encripta la contraseña

                // Actualiza la contraseña en la base de datos para el usuario actual
                string comandoUpdate = "UPDATE usuario SET contrasena = @HashedPassword WHERE email = @Email";
                var parametrosUpdate = new DbParameter[]
                {
                    CreateParameter("@HashedPassword", hashedPassword),
                    CreateParameter("@Email", email)
                };

                controlConexion.EjecutarConsultaSql(comandoUpdate, parametrosUpdate);
                controlConexion.CerrarBd(); // Cierra la conexión al final del proceso
                return Ok("Contraseña de usuario@gmail.com encriptada exitosamente.");
            }
            else
            {
    ...
    public IActionResult VerificarContrasena(string projectName, string tableName, [FromBody] Dictionary < string, string > datos) // Verifica si la contraseña proporcionada coincide con la almacenada.
    {
      ...
      try {
        ...
        var resultado = controlConexion.EjecutarConsultaSql(comandoSQL, new DbParameter[] {
          parametro
        }); // Ejecuta la consulta SQL para obtener la contraseña.
        controlConexion.CerrarBd(); // Cierra la conexión a la base de datos.
    ...
    public IActionResult EjecutarProcedimientoAlmacenado(string procedureName, [FromBody] JsonElement body) {
      ...
      if (string.IsNullOrWhiteSpace(procedureName)) {
        return BadRequest(new {
          Mensaje = "El nombre del procedimiento es requerido."
        });
      }
      ...
      try {
        ...
        using(var command = new SqlCommand(procedureName, (SqlConnection) connection)) {
          ...
          return Ok(new {
            Mensaje = "Procedimiento almacenado ejecutado exitosamente.", FilasAfectadas = filasAfectadas
          });
        }
      } catch (SqlException sqlEx) {
        ...
        return StatusCode(500, new {
          Mensaje = "Error en la base de datos.", Detalle = sqlEx.Message
        });
      } catch (Exception ex) {
        controlConexion.CerrarBd(); // Asegura cerrar la conexión en caso de error general
        Console.WriteLine($"Error: {ex.Message}");
        return StatusCode(500, new {
          Mensaje = "Se presentó Un error:", Detalle = ex.Message
        });
      }
    }
  }
